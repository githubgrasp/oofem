...
int Disk::generatePoints()
{
    ...
    // Random ring on the circumference (Bolander-style: random angles, distance check).
    ...
    // Random interior fill, area-uniform in [0, radius - diam].
    ...
    for ( int i = 0; i < maxIter; ++i ) {
        ...
        if ( flag == 0 && grid->addVertex(random) ) {
            ...
            // Mirror across the boundary so the dual cells partition cleanly
            // along the circle.
