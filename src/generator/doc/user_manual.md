...
| `#@holedisk` | `<id> centre 2 <cx> <cy> radius <r> [refine <r>]` | 2D circular hole (void) — meshed differently from a solid inclusion (`#@interfacedisk`): mechanical nodes are placed **on** the hole/matrix rim at **regular angular spacing** (a ring of `round(2·π·r / (refine·diam))` nodes on the circle, plus a paired ring one diameter inside), with sacrificial points filling the interior. Regular seeding (rather than random rejection) guarantees a gap-free rim polygon — `refine` sets the rim node count deterministically (smaller `refine` → more rim nodes). The sacrificial/inner points only steer the Delaunay so the rim comes out as a clean polygon; the converter drops everything strictly inside. Matrix fill points that would land inside any hole are rejected (the circular analog of `#@notch`). The converter declares the same hole via `#@holedisk` (or `#@diskinclusion … delete`); an annulus is a `#@disk` + one `#@holedisk`, and any number of holes may be placed in a `#@rect` or `#@disk` domain. |
