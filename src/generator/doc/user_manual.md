...
| `#@holedisk` | `<id> centre 2 <cx> <cy> radius <r> [refine <r>]` | 2D circular hole (void) — meshed differently from a solid inclusion (`#@interfacedisk`): mechanical nodes are placed **on** the hole/matrix rim (a ring on the circle plus a paired ring one diameter inside), with sacrificial points filling the interior. The sacrificial/inner points only steer the Delaunay so the rim comes out as a clean polygon; the converter drops everything strictly inside. Matrix fill points that would land inside any hole are rejected (the circular analog of `#@notch`). Use `refine < 1` to densify the rim — a coarse rim can leave a few tangential transport edges on the converter side. The converter declares the same hole via `#@holedisk` (or `#@diskinclusion … delete`); an annulus is a `#@disk` + one `#@holedisk`, and any number of holes may be placed in a `#@rect` or `#@disk` domain. |
