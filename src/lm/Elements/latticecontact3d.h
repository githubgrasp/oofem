...
#ifndef latticecontact3d_h
#define latticecontact3d_h

#include "latticestructuralelement.h"

///@name Input fields for LatticeContact3d
//@{
...
namespace oofem {
/**
 * Node-to-node contact/interface element for coupling to continuum meshes.
 *
 * Two nodes carry only translational DOFs (no rotations), so both ends can be
 * continuum nodes. The element evaluates the translational displacement jump
 * in a local frame (normal + two shears) and passes it as a three-component
...
public:
    LatticeContact3d(int n, Domain *);
    virtual ~LatticeContact3d();
    ...
    const char *giveInputRecordName() const override { return _IFT_LatticeContact3d_Name; }
    const char *giveClassName() const override { return "LatticeContact3d"; }
