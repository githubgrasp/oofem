...
public:
    ...
    /// Enable a diagnostic trace of the ellipsoid placement loop. When set,
    /// every trial in `placeOne` is appended to `path` — the rejected
    /// candidates (which overlap an existing aggregate) as well as the one
    /// finally accepted. This is an observer only: it logs what the loop
    /// already computes without touching control flow or the RNG, so the
    /// resulting packing is byte-identical whether tracing is on or off.
    /// Intended for illustrating the trial-and-error process, not production.
...
private:
    ...
    /// Append one trial record to the trace file (no-op if tracing is off).
    /// `status` is "overlap", "boundary", or "accept"; `id` is the id the
    /// aggregate will receive on success; `iter` is the 0-based attempt index.
    void logTrial(int id, int iter, const char *status,
