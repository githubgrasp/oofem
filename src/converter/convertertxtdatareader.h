...
#ifndef convertertxtdatareader_h
#define convertertxtdatareader_h
 ...
 */
class ConverterTXTDataReader : public ConverterDataReader
{
protected:
    std::string dataSourceName;
    std::list< ConverterTXTInputRecord >recordList;

    /// Keeps track of the current position in the list
    std::list< ConverterTXTInputRecord >::iterator it;

public:
    /// Constructor.
    ConverterTXTDataReader(std::string inputfilename);
    ConverterTXTDataReader(const ConverterTXTDataReader &x);
    virtual ~ConverterTXTDataReader();

    ConverterInputRecord &giveInputRecord(ConverterInputRecordType, int recordId) override;
