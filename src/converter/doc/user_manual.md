...
|-----------|-----------|---------|
| `#@grid` | `<type>` | Selects the output generator. Available types: `3dSM`, `3dTM`, `2dSM`, `2dTM`, `2dSMTM`. The 3D writers cover plain, periodic, and (3D-SM only) fibre-bearing analyses; the 2D writers cover plain SM (with optional periodicity via `latticeboundary2d`) and plain TM. **`2dSMTM`** writes BOTH coupled subproblems in one pass from a single `control.in` plus a `#@tmcontrol <file>` pointing to the transport template: it emits `oofem.sm.in` (mechanical, Delaunay) and `oofem.tm.in` (transport, Voronoi) with shared node numbering, so `#@coupling … neumann` and `#@couplingflag` can cross-reference SM and TM nodes/elements directly. (A 3D staggered SMTM analysis is still expressed as two separate templates, one per subproblem, each with its own `#@grid`.) See `Grid::resolveGridType`. |
| `#@tmcontrol` | `<file>` | Path to the transport (TM) control template; used only with `#@grid 2dSMTM`. The converter reads the SM template (`control.in`, the file it was invoked on) **and** this TM template, and writes `oofem.sm.in` + `oofem.tm.in` together with shared node numbering. The TM template carries the transport engineering model, material, BCs and the `{#@COUPLINGBC}`-style records on its own side. |
...
| `#@rect` | `<id> box 4 <xmin> <ymin> <xmax> <ymax>` | 2D analog of `#@prism` — axis-aligned rectangle. One of `#@rect` / `#@disk` is required as region 1 for `#@grid 2dSM` / `2dTM` / `2dSMTM`; the converter uses it for boundary tests, Voronoi-vertex projection, and the inside-domain element filter. For a circular outer domain use `#@disk`. |
| `#@disk` | `<id> centre 2 <cx> <cy> radius <r>` | 2D circular domain region — the converter counterpart of the generator's `#@disk` and the 2D analog of `#@cylinder`. Usable as region 1 for `#@grid 2dSM` / `2dTM` / `2dSMTM`. Classifies vertices by radial distance (inside / on-circle / outside) and projects the Voronoi cross-section vertices of boundary mechanical elements radially onto the circle, so transport nodes lie on the boundary with no tangential transport edge. The disc is solid; holes (annulus, multiple voids) are separate `#@holedisk` inclusions. Non-periodic only. |
...
| `#@diskinclusion` | `<id> centre 2 <cx> <cy> radius <r> itz <t> inside <mi> interface <mif>` <br>or `<id> centre 2 <cx> <cy> radius <r> delete` | 2D analog of `#@sphereinclusion` — circular inclusion with ITZ halo (`itz`/`inside`/`interface` form). Internally stored as `SphereInclusionSpec` with `cz = 0`; the existing 3D midpoint / straddle classification works for 2D points unchanged because every 2D vertex has `z = 0`. <br>With the `delete` keyword instead it declares a **hole/void** — equivalent to `#@holedisk` (see below). |
| `#@holedisk` | `<id> centre 2 <cx> <cy> radius <r>` | Circular hole (void) — a first-class `HoleDisk`. The same line works in the generator and converter input files (the converter spelling of `#@diskinclusion … delete`). The generator seeds mechanical nodes on the hole rim plus sacrificial interior points; the converter drops everything strictly inside the hole and shifts the dual cross-section (Voronoi/transport) vertices of the rim edges radially onto the circle, leaving mechanical **and** transport nodes on the boundary with no tangential transport edge. Any number of holes may be declared, inside a `#@rect` or `#@disk` domain. (Densify the rim with the generator-side `refine` token if a coarse rim leaves stray tangential transport edges.) |
| `#@cylinderinclusion` | `<id> line 6 <x1> <y1> <z1> <x2> <y2> <z2> radius <r> itz <t> inside <mi> interface <mif> [debond <s0> <s1> <matDebond>]` | Straight-axis cylindrical inclusion with ITZ halo. The classification uses perpendicular distance from each endpoint to the infinite axis through the two line points; inclusion semantics match `#@sphereinclusion`. The optional `debond <s0> <s1> <matDebond>` gives interface elements whose midpoint axial position `s` (arclength measured from point 1 along the axis) lies in `[s0, s1]` the material `matDebond` instead of `mif` — e.g. a bond-free / frictionless sleeve near the loaded face of a pull-out specimen. Use-case: rebar + ITZ + matrix partitioning for corrosion-cracking and bond/pull-out studies. |
...
| `#@coupling` | `hole <id> ltf <ltf_id> { pressure <p> \| neumann tmbc <bc_id> }` *or* `inclusion <id> dirichlet ltf <ltf_id>` | Hydro-mechanical coupling. **Hole forms** (GraFahGalWhe15 thick-walled cylinder, *transport → mechanical*) act on the rim of `#@holedisk <id>`; records are spliced into `control.in` at the `{#@COUPLINGBC}` / `{#@COUPLINGSET}` placeholders, with `nbc`/`nset` bumped automatically. Rigid-body restraint is the user's responsibility (e.g. tangential pins at three `#@controlvertex` nodes). <br>• **`pressure <p>`** (Approach 1, SM-only, standalone) — for each rim mechanical node, an outward radial `NodalLoad` (`f = p · tributary · radial`) plus a single-node `Set`, pressure scaled by `<ltf_id>`. <br>• **`neumann tmbc <bc_id>`** (staggered SMTM) — one `LatticeNeumannCoupling` active BC per rim node, reading the live transport pore pressure and applying it as the inward boundary traction; `tmbc <bc_id>` tags the dual transport rim nodes with `bc 1 <bc_id>` so the TM template's `BoundaryCondition <bc_id>` prescribes the inner pressure. Driver `coupling 3 2 1 0`. Pairs with `#@couplingflag` + `latticelinearelastic … bio <b>` for the distributed Biot term. <br>• **`inclusion <id> dirichlet ltf <ltf_id>`** (Approach 2, displacement-driven, *mechanical → transport*) — for the ITZ of `#@diskinclusion <id> … itz …`: each radial ITZ structural element (a Delaunay edge straddling the inclusion circle) has a dual transport element whose two midline nodes receive the pore pressure. The writer emits, **into the TM file** at the `#@couplingbc` / `#@couplingset` marker lines, one `LatticeDirichletCoupling` BC per midline transport node (`couplingelements 2 <e1> <e2>` = its two flanking radial ITZ elements, `dofs 1 11` = P_f) plus a single-node `Set`, and bumps the TM `nbc`/`nset`. Requires `#@grid 2dSMTM`; the SM subproblem is written first so the BCs can reference SM element ids. Driver `coupling 3 1 2 0` (**SM-first**, the opposite of the Neumann order — the mechanical stress must exist before the transport reads it). The ITZ eigenstrain is applied separately via `#@bodyload` + a `StructTemperatureLoad`. |
...
| `#@slaveside` | `<master_ctl_id> face <axis> <min\|max> dofs <list> [inclusiononly]` | SM-only. Like `#@rigidarm` but uses `DT_simpleSlave` (identical-value slaving) instead of rigid-arm kinematics — the slave DOF takes exactly the master's value, with no rotation transfer. Each inside Delaunay vertex on the specified face is emitted as `node … dofidmask … doftype <flags> mastermask <ids>`, slaved to the controlvertex `<master_ctl_id>`. Only the DOFs listed after `dofs` are slaved (entries: `1`=D_u, `2`=D_v, `3`=D_w, `4`=R_u, `5`=R_v, `6`=R_w; for 2D lattice nodes the writer emits `dofidmask 3 1 2 6` and slaves the listed subset). The optional `inclusiononly` keyword restricts slaving to nodes inside an inclusion (perp distance from an inclusion axis/centre `< radius + itz/2`) — the complement of `#@nodebc … matrixonly`. Every named `#@controlvertex` (any id, not just the spec's own master) is exempt from slaving and stays a regular node, so corner anchors and observer points remain free regardless of which face they sit on. Works for both 2D (`#@rect`) and 3D (`#@prism`) regions; multiple `#@slaveside` directives may be combined (one per face/master). Use-case: pulling one face of a non-periodic specimen uniformly in a single direction (e.g. direct-tension test where the right face follows a load-controlled master node), or slaving only the reinforcement nodes on a face so a single master's reaction gives the total pull-out force (`inclusiononly`, paired with `#@nodebc … matrixonly` to clamp the surrounding matrix). |
| `#@nodebc` | `<bc_id> face <axis> <min\|max> [matrixonly] [dofs <n> <d1..dn>]` | Tags every emitted node lying on the named face plane of region 1's bounding box (`axis` in {1,2,3} for x/y/z) so OOFEM applies the listed `BoundaryCondition` records to those nodes. Multiple directives stack — repeating with the same face adds another id; a node sitting on two requested faces (e.g. a corner) collects both. In the **3D-SM writer** the tag is emitted as a full per-DOF array `bc 6 b1..b6`, placing `<bc_id>` at each constrained DOF position and `0` elsewhere (OOFEM requires one entry per DOF of a 6-DOF lattice node); `dofs` selects which DOFs are constrained (default: the three translations `1 2 3`). The referenced `BoundaryCondition` should therefore be a scalar `prescribedvalue` record. The optional `matrixonly` keyword tags only matrix nodes — those outside every `#@sphere/cylinderinclusion` — leaving bar/interface nodes that share the same face plane free (e.g. the support face of an embedded-reinforcement pull-out specimen). (In the TM writer nodes carry a single DOF and the legacy `bc 1 <id>` form is used.) Skipped on nodes that are themselves slaved by `#@slaveside` (mixing `bc` with `mastermask` is ill-defined in OOFEM); the periodic rigid-body pin in 3D TM/SM writers is merged with `#@nodebc` ids on the first emitted node. Use-case: prescribing saturation on top/bottom of a transport mesh, or clamping the concrete support face (translations) of a non-periodic SM specimen, without a post-process script. |
| `#@edgebc` | `region <id> bc <bc_id>` | Tags every emitted boundary node lying on the **edge of region `<id>`** with `bc 1 <bc_id>`, applying that `BoundaryCondition` along the whole region boundary (unlike `#@nodebc`, which is limited to axis-aligned bounding-box faces, this follows the region's true shape). Resolution uses the region's `onBoundary(x,y,tol)` query, so it works for any region type — `#@disk` → the outer circle, `#@rect` → any of the four edges. Currently wired in the 2D TM writer (one `P_f` dof per node); face (`#@nodebc`), edge (`#@edgebc`) and hole-rim (`#@coupling`) tags merge into a single `bc` record per node. Use-case: holding a disk's **outer boundary at zero pore pressure** (drainage) so the steady-state field of a thick-walled cylinder becomes logarithmic — without it the outer boundary is insulated and the field is uniform. |
| `#@lumpedcapacity` | `<0\|1>` | TM-only. When `1`, every emitted `latticemt2D` / `latticemt3D` / `latticemt3Dboundary` element line gets `lumpedcapacity 1` appended, switching the element capacity matrix from the consistent (coupled) form to a TPFA-style monotone diagonal form. Required for nonlinear `c(p)` — e.g. unsaturated Richards-style flow with retention curves — to avoid spurious oscillations near sharp wetting fronts. For `latticemt3Dboundary` only the upper-left 2×2 (the two regular nodes) is lumped; the macro-gradient block on the third node has no storage term and stays zero. Default `0`. |
...
| `{#@COUPLINGBC}` | *(inline placeholder)* | Replaced with the `NodalLoad` records generated by `#@coupling` (one per hole-rim mechanical node). Put it in the BC section of `control.in`; empty if `#@coupling` is absent. |
| `{#@COUPLINGSET}` | *(inline placeholder)* | Replaced with the single-node `Set` records generated by `#@coupling` (one per hole-rim mechanical node). Put it in the set section of `control.in`; empty if `#@coupling` is absent. |
| `#@couplingbc` | *(marker line)* | Used by `#@coupling inclusion … dirichlet`. Replaced with the `LatticeDirichletCoupling` BC records (one per midline transport node) in the **TM template** (`#@tmcontrol` file). Put it in the BC section — after the materials, before the load-time functions; empty if no Dirichlet coupling. |
| `#@couplingset` | *(marker line)* | Used by `#@coupling inclusion … dirichlet`. Replaced with the single-node `Set` records (one per midline transport node) in the **TM template**. Put it at the end of the set section; empty if no Dirichlet coupling. |
...
| `#@vtk` | *(no arguments)* | Opt in to writing the ParaView `.vtu` files (`*.voronoielement.vtu`, `*.delaunayelement.matN.vtu`, `*.fibre.beamElement.vtu`, etc.). Default off — VTK files are not generated unless this directive is present. One `*.delaunayelement.matN.vtu` is written per structural material declared in `control.in` (`nmat`); each element's material is resolved on the fly from the notch/inclusion geometry, so the mesh can be shown with projected materials without running an analysis. |
...
| `#@element` | `<entityKind> <entityID> <elementName> <crossSect> <mat>` *or* `material <mat> <elementName>` | Element-name override. **Entity form** (T3D writer): `<entityKind>` is one of `vertex`, `curve`, `surface`, `patch`, `shell`; edges classified to the given entity emit `<elementName>` instead of the writer's hardcoded default (`lattice3D`, `lattice3d`, or `lattice3Dnl` depending on geometry). Edges that don't match any directive use the default. Resolution priority for an edge that touches multiple entities: curve > surface > region. **Material form** (Delaunay/qhull `give3DSMOutput` writer, which has no T3D entities): every inside line resolving to material `<mat>` emits `<elementName>` instead of `lattice3D`. Since that writer already emits `crossSect == mat == <mat>` with the Voronoi facet as `polycoords`, this is exactly the record a `latticecontact3d` interface element expects — so `#@element material 3 latticecontact3d` turns the ITZ shell of a `#@cylinderinclusion` into node-to-node bond-contact elements (pair with a `latticebondplast` material and a `latticecs` for material 3). |
| `#@rebar` | `<curveID> diameter <d> crossSect <cs> mat <m> bondCS <bcs> bondMat <bmat> [element <name>]` | Declare a T3D curve as an embedded reinforcement bar. The curve must be tagged with `output yes` in `mesh.in` so T3D emits its 1D line segments. The rebar curve should use its own vertices declared independently of the patch — T3D shares mesh nodes only between entities that explicitly reference each other (e.g. a curve's endpoint vertices, or a patch's boundary curves), so geometric coincidence alone does not cause merging. The converter emits one rebar element per curve segment (default `lattice3Dnl`; override with `element <name>`) using `crossSect`/`mat`, plus one `latticelink3D` per rebar node coupling it to the nearest matrix Delaunay vertex. The link's `length` is the tributary rebar-segment length at that node (interior: average of adjacent segments; end: half the single adjacent segment); `dirvector` is the local rebar tangent; `diameter` is the rebar diameter from the directive; bond cross-section and material come from `bondCS`/`bondMat`. The rebar `LatticeCS` should normally use `shape 1 radius <r>` for a circular cross-section. Use-case: discrete reinforcement in plain or textile-reinforced concrete shells with MC2010 bond-slip via `latticeslip`. Multiple `#@rebar` directives are allowed (one per bar curve). |
