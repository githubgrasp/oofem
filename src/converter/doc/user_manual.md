...
| `#@nodebc` | `<bc_id> face <axis> <min\|max>` | Tags every emitted node lying on the named face plane of region 1's bounding box (`axis` in {1,2,3} for x/y/z) with `bc <n> <id1> <id2>…`, so OOFEM applies the listed `BoundaryCondition` records to those nodes. Multiple directives stack — repeating with the same face adds another id; a node sitting on two requested faces (e.g. a corner) collects both. The bc tag is appended to the regular `node …` line as it is written, including for TM and SM (2D/3D) writers. Skipped on nodes that are themselves slaved by `#@slaveside` (mixing `bc` with `mastermask` is ill-defined in OOFEM); the periodic rigid-body pin in 3D TM/SM writers is merged with `#@nodebc` ids on the first emitted node. Use-case: prescribing saturation on top/bottom of a transport mesh, or zero-displacement on the bottom of a non-periodic SM specimen, without a post-process script. |
