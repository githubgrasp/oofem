...
`#@grid 3dSM` dispatches to a single writer (`Grid::give3DSMOutput`) that handles plain SM, periodic SM, and periodic SM with fibres, driven entirely by `#@perflag` and the presence of `#@fibre` records. The previously separate `3dFPZ`, `3dFPZFibre`, `3dFibreBenchmark`, `3dGopSha`, `3dTension`, and `3dSphere` grid types have been retired; their analyses are expressed entirely as `control.in` templates using `#@notch` / `#@sphereinclusion` for per-region material overrides and `#@controlvertex` / `#@CTL<id>` for support/load points.
