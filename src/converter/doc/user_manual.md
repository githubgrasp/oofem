...
| `#@sphereinclusion` | `<id> centre 3 <x> <y> <z> radius <r> itz <t> inside <mi> interface <mif> [bodyload <b>]` | Spherical inclusion with ITZ halo. Effective radius is `r + t/2`. An element whose endpoints both fall inside gets `crossSect mi mat mi`; an element whose endpoints straddle the sphere boundary (one inside, one outside — the ITZ zone) gets `crossSect mif mat mif` and, if `bodyload` is given, a `bodyloads 1 <b>` suffix. Applied after `#@notch` on the inherited default material. The control file is expected to declare matching `latticecs` / material pairs for both `mi` and `mif` and, if needed, a matching `BoundaryLoad`/`StructTemperatureLoad` for `<b>`. |
| `#@cylinderinclusion` | `<id> line 6 <x1> <y1> <z1> <x2> <y2> <z2> radius <r> itz <t> inside <mi> interface <mif> [bodyload <b>]` | Straight-axis cylindrical inclusion with ITZ halo. The classification uses perpendicular distance from each endpoint to the infinite axis through the two line points; inclusion/interface/bodyload semantics match `#@sphereinclusion`. Use-case: rebar + ITZ + matrix partitioning for corrosion-cracking studies where a thermal load is applied only to the ITZ. |
| `#@meshtype` | `tetra` \| `lattice` | Switches the `3dSM` writer from the default lattice (Voronoi-edge `lattice3D`) mode to tetrahedral-element (`ltrspace`) mode. In `tetra` mode the converter expects a 3-file qhull pipeline: `mesh.nodes` + `mesh.delaunay` (from `qdelaunay Qt i < mesh.nodes`) + `mesh.voronoi` (still needed for the localiser). Each inside Delaunay tetrahedron is emitted as `ltrspace` with 4 node ids and `crossSect 1 mat 1`. Also sets `regularFlag = 2` so `Region::findOutsiders` flags tetrahedra instead of lines. |
...
`#@grid 3dSM` dispatches to a single writer (`Grid::give3DSMOutput`) that handles plain SM, periodic SM, and periodic SM with fibres, driven entirely by `#@perflag` and the presence of `#@fibre` records. The previously separate `3dFPZ`, `3dFPZFibre`, `3dFibreBenchmark`, `3dGopSha`, `3dTension`, `3dSphere`, `3dCylinder`, and `3dTetraSM` grid types have been retired; their analyses are expressed entirely as `control.in` templates using `#@notch` / `#@sphereinclusion` / `#@cylinderinclusion` for per-region material overrides, `#@meshtype tetra` for solid-tetrahedron output, and `#@controlvertex` / `#@CTL<id>` for support/load points.
