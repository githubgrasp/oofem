...
| `#@rect` | `<id> box 4 <xmin> <ymin> <xmax> <ymax>` | 2D analog of `#@prism` — axis-aligned rectangle. One of `#@rect` / `#@disk` is required as region 1 for `#@grid 2dSM` / `#@grid 2dTM`; the converter uses it for boundary tests, Voronoi-vertex projection, and the inside-domain element filter. For a circular outer domain use `#@disk`. |
| `#@disk` | `<id> centre 2 <cx> <cy> radius <r>` | 2D circular domain region — the converter counterpart of the generator's `#@disk` and the 2D analog of `#@cylinder`. Usable as region 1 for `#@grid 2dSM` / `#@grid 2dTM`. Classifies vertices by radial distance (inside / on-circle / outside) and projects the Voronoi cross-section vertices of boundary mechanical elements radially onto the circle, so transport nodes lie on the boundary with no tangential transport edge. The disc is solid; holes (annulus, multiple voids) are separate `#@holedisk` inclusions. Non-periodic only. |
...
| `#@coupling` | `hole <id> ltf <ltf_id> pressure <p>` | Hydro-mechanical boundary coupling on the rim of `#@holedisk <id>` (GraFahGalWhe15 thick-walled cylinder, Approach 1). SM-only. For each rim mechanical node it emits an outward radial `NodalLoad` (`f = p · tributary · radial`) and a single-node `Set`, spliced into `control.in` at the `{#@COUPLINGBC}` / `{#@COUPLINGSET}` placeholders; `nbc` / `nset` on the counts line are bumped automatically (so the template's base counts must include any anchor BCs/sets). The reference pressure `<p>` is scaled by load-time function `<ltf_id>`. Rigid-body restraint is the user's responsibility — e.g. tangential pins at three `#@controlvertex` nodes applied via sets in `control.in`. (This emits direct radial loads; the staggered transport-driven `LatticeNeumannCoupling` form is future work.) |
