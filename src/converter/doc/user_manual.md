...
| `#@coupling` | `hole <id> ltf <ltf_id> { pressure <p> \| neumann tmbc <bc_id> }` | Hydro-mechanical boundary coupling on the rim of `#@holedisk <id>` (GraFahGalWhe15 thick-walled cylinder). Records are spliced into `control.in` at the `{#@COUPLINGBC}` / `{#@COUPLINGSET}` placeholders; `nbc` / `nset` on the counts line are bumped automatically (so the template's base counts must include any anchor BCs/sets). Rigid-body restraint is the user's responsibility — e.g. tangential pins at three `#@controlvertex` nodes applied via sets in `control.in`. Two forms: <br>• **`pressure <p>`** (Approach 1, SM-only, standalone) — for each rim mechanical node it emits an outward radial `NodalLoad` (`f = p · tributary · radial`) plus a single-node `Set`, with the reference pressure scaled by load-time function `<ltf_id>`. <br>• **`neumann tmbc <bc_id>`** (staggered SMTM) — emits one `LatticeNeumannCoupling` active BC linking the rim mechanical nodes to their dual transport rim nodes; it reads the live pore pressure at those transport nodes (the transport slave problem) and applies it as the inward boundary traction. The companion `tmbc <bc_id>` tags the same rim nodes in the **transport** input with `bc 1 <bc_id>`, so the TM template's `BoundaryCondition <bc_id>` prescribes the inner pressure there. Use with `#@grid 2dSMTM` and a `StaggeredProblem` driver (`coupling 3 2 1 0`). Pairs with `#@couplingflag` + `latticelinearelastic … bio <b>` for the distributed Biot term. |
