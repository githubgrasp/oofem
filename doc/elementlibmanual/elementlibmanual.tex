...
Represents two-node lattice element for small rotations. Each node has 3 degrees of freedom.
...
Lattice3d represents a two-node 3d lattice element for small rotations. Each node has six degrees of freedom as shown in Figure~\ref{lattice3dfig}.
...
\begin{elementsummary}{lattice3d}{3d lattice element}{\field{polycoords}{ra}  \field{shellnormal}{ra} \field{couplingflag}{in} \field{couplingnumbers}{ra} \field{pressures}{ra} \field{mlength}{rn}}{lattice3d element summary}{lattice3dsummary}
...
\elementParam{\param{shellnormal}: three-component array (optional parameter) giving the shell surface normal in global coordinates. Supply this keyword on every lattice element that represents a shell so that the local cross-section frame is aligned with the through-thickness direction.}
...
This element represents a two-node 3d link element connecting 3d beam (rebar/fibre) and 3d lattice (matrix) elements. Each node has six degrees of freedom.
The input parameters for this element are shown in Table~\ref{latticelink3dsummary}.

\textbf{Node ordering}: node~1 is the \emph{matrix} lattice node; node~2 is the \emph{rebar} (beam) node. The Gauss point and all output quantities are reported at the rebar node (node~2). The rigid-body arm accounting for the offset between the two nodes is applied to the matrix node (node~1), so that matrix rotations are correctly included in the computed bond-slip.
...
\elementParam{\param{mlength}: minimum length (optional parameter) is used to check if the cross-section of the element is not too small. Default value is 1.e-20.}
\elementDescription{Unknowns}{Six dofs ($u$-displacement, $v$-displacement, $w$-displacement, $u$-rotation, $v$-rotation and $w$-rotation) are required in each node. Node~1 is the matrix lattice node; node~2 is the rebar node.}
...
\end{elementsummary}
 ...
 Latticeframe3d represents a two-node linear 3d frame element shown in Figure~\ref{fig:latticeframe3d}. Each node has six degrees of freedom, namely three translations and three rotations. The element is based on the link between rigid body spring and Timoshenko frame models. The main idea is to model the elastic and inelastic response of a connection of two nodes by a set of springs located at the contact facet of two rigid bodies. Displacement jumps are computed at the contact facet, which are smeared out over the element length in the form of strains. The input parameters for this element are shown in Table ~\ref{latticeframe3dsummary}. For the computation of the nodal forces, the initial configuration is used. The position of the plastic hinge is controlled by parameter $s$, which is normalised by half of the element length from the centre of the element. This approach was developed by \cite{Toi91}.
 
  \begin{figure}[htb]
    \centering
  \includegraphics[width=0.4\textwidth]{./latticeframe3d.png}
  \caption{latticeframe3d element. Node numbering, DOF numbering and location of contact with springs.}
  \label{fig:latticeframe3d}
  \end{figure}

 \begin{elementsummary}{latticeframe3d}{3d lattice frame element}{\field{zaxis}{ra} \optField{s}{rn}}{latticeframe3d element summary}{latticeframe3dsummary}
\elementParam{\param{zaxis}: z-axis of local coordinate system }
\elementParam{\param{s}:  distance from centre to location of springs normalised by half of element length (0  centre of element (default), -1  left node of element, 1 right end of element)}
\elementDescription{CS properties}{Area, inertia moment along y and z axis (\param{iy} and \param{iz} parameters), torsion inertia moment (\param{ik} parameter) and either cross section area shear correction factor (\param{beamshearcoeff} parameter) or equivalent shear areas (\param{shearareay} and \param{shearareaz} parameters) are required. These cross section properties are assumed to be defined in local coordinate system of element.}
\elementDescription{Unknowns}{Six dofs ($u$-displacement, $v$-displacement, $w$-displacement, $u$-rotation, $v$-rotation and $w$-rotation) are required in each node.}
\elementDescription{Reference}{\cite{Toi91}}
\end{elementsummary}

 \subsubsection{Latticeframe3dnl element}
 Latticeframe3dnl represents a two-node geometrically nonlinear 3d frame element. This element is an extension of latticeframe3d. Each node has six degrees of freedom, namely three translations and three rotations. The element is based on the link between rigid body spring and Timoshenko frame models. The main idea is to model the elastic and inelastic response of a connection of two nodes by a set of springs located at the contact facet of two rigid bodies. Displacement jumps are computed at the contact facet using the deformed geometry assuming large deformations, which are smeared out over the element length in the form of strains. The input parameters for this element are shown in Table~\ref{latticeframe3dnlsummary}. For the computation of the nodal forces, the deformed configuration is used as illustrated in Figure~\ref{fig:latticeframe3dnl}.

 \begin{figure}[htb]
    \centering
  \includegraphics[width=0.5\textwidth]{./latticeframe3dnl.png}
  \caption{latticeframe3dnl element. Illustration of calculation of nodal forces using deformed geometry.}
  \label{fig:latticeframe3dnl}
  \end{figure}
 
 \begin{elementsummary}{latticeframe3dnl}{3d lattice frame element}{-}{latticeframe3dnl element summary}{latticeframe3dnlsummary}
   \elementParam{}
\elementDescription{CS properties}{Area, inertia moment along y and z axis (\param{iy} and \param{iz} parameters), torsion inertia moment (\param{ik} parameter) and either cross section area shear correction factor (\param{beamshearcoeff} parameter) or equivalent shear areas (\param{shearareay} and \param{shearareaz} parameters) are required. These cross section properties are assumed to be defined in local coordinate system of element.}
\elementDescription{Unknowns}{Six dofs ($u$-displacement, $v$-displacement, $w$-displacement, $u$-rotation, $v$-rotation and $w$-rotation) are required in each node.}
...
\element{latticemt3d}; the third is a control node that supplies the macro
...
\element{lattice3dboundary} (Figure~\ref{lattice3dboundaryfig}).
...
\elementParam{\param{polycoords}: coordinates of the mid-cross-section (Voronoi facet) vertices in the global system.}
\elementParam{\param{location}: array of two integers between 1 and 26 specifying the location of the two transport nodes with respect to the 3D periodic cell — same encoding as \element{lattice3dboundary}.}
\elementParam{\param{dim}: dimension factor (optional, default 3).}
\elementParam{\param{area}: prescribed cross-section area (optional; otherwise computed from \param{polycoords}).}
\elementParam{\param{mlength}: minimum element length threshold (optional).}
\elementParam{\param{crackwidths}: per-vertex crack widths used in conductivity scaling (optional).}
...
\elementDescription{Unknowns}{Single dof (P\_f) per transport node. The third (control) node carries the periodic-cell dofs as for \element{lattice3dboundary} but reduced to the transport-relevant components.}
\elementDescription{Approximation}{Linear pressure along the element.}
\elementDescription{Integration}{One Gauss point at the element midpoint.}
\elementDescription{CS properties}{simplecs.}
\elementDescription{Status}{Reliable}
