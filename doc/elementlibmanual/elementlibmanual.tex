...
The same element can also represent a two-node beam (frame) member. In beam mode no polygon vertices are given; instead the cross-section properties (area, second moments of area $iy$ and $iz$, torsion constant $ik$ and shear properties) are taken directly from the lattice cross-section, and the orientation of the local transverse axes is set by the \param{zaxis} parameter. The position of the integration point (contact facet, i.e. the location of the plastic hinge) along the element is controlled by \param{s}. If $iy=iz$ the \param{zaxis} may be omitted. The beam formulation follows the rigid-body-spring/Timoshenko frame link of \cite{Toi91}.
...
\begin{elementsummary}{lattice3d}{3d lattice element}{\field{polycoords}{ra}  \field{shellnormal}{ra} \optField{zaxis}{ra} \optField{s}{rn} \field{couplingflag}{in} \field{couplingnumbers}{ra} \field{pressures}{ra} \field{mlength}{rn}}{lattice3d element summary}{lattice3dsummary}
...
\elementParam{\param{zaxis}: three-component array giving the local $z$-axis of a beam cross-section (optional; required only when $iy \neq iz$). Not used for facet/shell cross-sections, and mutually exclusive with \param{shellnormal}.}
\elementParam{\param{s}: position of the integration point along a beam element, normalised by half the element length ($0$: centre of element (default), $-1$: first node, $1$: second node). Beam mode only.}
\elementDescription{CS properties (beam mode)}{Area, second moments of area along the local $y$ and $z$ axes (\param{iy} and \param{iz}), torsion constant (\param{ik}) and either a cross-section shear correction factor (\param{beamshearcoeff}) or equivalent shear areas (\param{shearareay} and \param{shearareaz}) are taken from the lattice cross-section, defined in the local coordinate system of the element.}
...
\elementParam{\param{polycoords}, \param{shellnormal}, \param{zaxis}, \param{s}: as for the \element{lattice3d} element.}
\elementDescription{CS properties (beam mode)}{As for the \element{lattice3d} element.}
\elementDescription{Unknowns}{Six dofs ($u$-displacement, $v$-displacement, $w$-displacement, $u$-rotation, $v$-rotation and $w$-rotation) are required in each node.}
